...
def load_checkpoint(filepath, arch):
    ...
    if checkpoint['arch'] == arch:
        ...
        print('info : sorry. the arch dosen\'t match the checkpoint. you need to train a new model.')
        sys.exit(5)
...
def process_image(image):
    ''' this function scales, crops, and normalize a PIL image for the PyTorch model, 
...
def predict(image_path, model, topk=5, gpu=True):
    ...
    if gpu and torch.cuda.is_available():
        device = 'cuda'
        print('info : use gpu to predict')
    elif gpu == True and torch.cuda.is_available() == False:
        device = 'cpu'
        print('info : gpu is not available, use cpu to predictt')
    else:
        device = 'cpu'
        print('info : use cpu to predict')
...
if __name__ == '__main__':
    ...
    if args.arch not in arch_list:
        print('info : only support {}'.format(arch_list))
        sys.exit(2)
    ...
    if os.path.exists(args.checkpoint):
        ...
        print('info : checkpoint file not exists')
        sys.exit(4)
